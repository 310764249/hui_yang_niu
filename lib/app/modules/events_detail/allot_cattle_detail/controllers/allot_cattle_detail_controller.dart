...
class AllotCattleDetailController extends GetxController {
  //TODO: Implement AllotCattleDetailController
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      var response = await httpsClient.get(
        "/api/allot/$businessId",
      );
      event = AllotEvent.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
