...
class MapTools {
  /// 百度地图
  static Future<bool> gotoBaiduMap(longitude, latitude, {String? toAddress}) async {
    var url =
    ...
    if (!canLaunchUrl) {
      Toast.failure(msg: '未检测到百度地图~');
      return false;
    }
    //
    canLaunchUrl = await launchUrlString(url);
  ...
  /// 高德地图
  static Future<bool> gotoGaoDeMap(longitude, latitude,
      {String? toAddress}) async {
        
    List<num> list = GpsUtil.bd09_To_Gcj02(num.parse(latitude), num.parse(longitude));
    var url =
  ...
  /// 腾讯地图
  static Future<bool> gotoTencentMap(longitude, latitude,
      {String? toAddress}) async {
    List<num> list = GpsUtil.bd09_To_Gcj02(num.parse(latitude), num.parse(longitude));
    var url =
  ...
  static Future<bool> gotoAppleMap(longitude, latitude,
    ...
    List<num> list = GpsUtil.bd09_To_Gcj02(num.parse(latitude), num.parse(longitude));
    var url =
        'http://maps.apple.com/?daddr=${list[0]},${list[1]}&address=$toAddress';
    url = Uri.encodeFull(url);
    print('url=$url');
    //
    bool canLaunchUrl = await launchUrlString(url);
    if (!canLaunchUrl) {
      Toast.failure(msg: '打开失败~');
      return false;
    }
