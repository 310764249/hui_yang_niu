...
class PregcyController extends GetxController {
  //TODO: Implement PregcyController
  //传入的参数
  ...
  void onInit() {
    ...
    //初始化字典项
    passList = AppDictList.searchItems('yjjg') ?? [];
  ...
  void handleArgument() async {
    ...
    if (argument is Cattle) {
      ...
      isEdit.value = true;
      //编辑
      event = PregcyEvent.fromJson(argument.data);
      //填充耳号
      ...
      curPassIndex.value = AppDictList.findIndexByCode(
          passList, event!.result.toString()); //显示选中项
      //填充时间
  ...
  void requestCommit() async {
    ...
    if (timesStr.value.isBefore(selectedCow.inArea)) {
      Toast.show('孕检时间不能早于入场日期');
      return;
    }
    //时间不能小于出生日期
    if (timesStr.value.isBefore(selectedCow.birth)) {
      Toast.show('孕检时间不能早于出生日期');
      return;
    }
  ...
  void newAction() async {
    ...
    try {
      ...
      Map<String, dynamic> para = {
        ...
        'result':
            int.parse(curPassID), //必传 integer 孕检结果1：返情；2：流产；3：B超阴性；4：B超阳性；
      ...
      // print(para);
      await httpsClient.post("/api/pregcy", data: para);
      Toast.dismiss();
  ...
  void editAction() async {
    ...
    try {
      ...
      // print(para);
      await httpsClient.put("/api/pregcy", data: para);
      Toast.dismiss();
      Toast.success(msg: '提交成功');
      Get.back();
    } catch (error) {
