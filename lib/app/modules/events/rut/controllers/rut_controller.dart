...
class RutController extends GetxController {
  //TODO: Implement RutController
  //传入的参数
  ...
  void onInit() {
    ...
    //初始化字典项
    passList = AppDictList.searchItems('fqbx') ?? [];
  ...
  void handleArgument() async {
    ...
    if (argument is Cattle) {
      ...
      isEdit.value = true;
      //编辑
      event = RutArgument.fromJson(argument.data);
      ...
      //是否明显
      curPassID = event!.present.toString(); //提交数据
      curPassIndex.value = AppDictList.findIndexByCode(
          passList, event!.present.toString()); //显示选中项
      //填充时间
  ...
  void requestCommit() async {
    ...
    if (timesStr.value.isBefore(selectedCow.inArea)) {
      Toast.show('发情时间不能早于入场日期');
      return;
    }
    //时间不能小于出生日期
    if (timesStr.value.isBefore(selectedCow.birth)) {
      Toast.show('发情时间不能早于出生日期');
      return;
    }
  ...
  void newAction() async {
    ...
    try {
      ...
      Map<String, dynamic> para = {
        ...
        'present': int.parse(curPassID), //必传 integer 发情表现1：明显；2：不明显；
      ...
      //print(para);
      await httpsClient.post("/api/rut", data: para);
      Toast.dismiss();
  ...
  void editAction() async {
    ...
    try {
      ...
      //print(para);
      await httpsClient.put("/api/rut", data: para);
      Toast.dismiss();
      Toast.success(msg: '提交成功');
      Get.back();
    } catch (error) {
