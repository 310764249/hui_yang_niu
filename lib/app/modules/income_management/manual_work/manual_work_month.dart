...
class ManualWorkMonth extends StatefulWidget {
  const ManualWorkMonth({super.key});

  @override
  State<ManualWorkMonth> createState() => _ManualWorkMonthState();
}

class _ManualWorkMonthState extends State<ManualWorkMonth> with AutomaticKeepAliveClientMixin {
  ...
  List<ManualWorkMonthEntity> manualWorkDayList = [];

  //刷新控件
  late EasyRefreshController refreshController;

  String api = '/api/manualwork/monthstatistics';
  ...
  Future getManualWorkDayStatistics({bool isRefresh = true}) async {
    ...
    try {
      ...
      List<ManualWorkMonthEntity> modelList = [];
      for (var item in mapList) {
        ManualWorkMonthEntity model = ManualWorkMonthEntity.fromJson(item);
        modelList.add(model);
      }
      if (mounted) {
        setState(() {
    ...
    return EasyRefresh(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return MonthProfitListView(data: item);
        },
      ),
    );
  }
