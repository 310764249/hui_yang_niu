...
class KnockOutDetailController extends GetxController {
  //TODO: Implement KnockOutDetailController
  //传入的参数
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      var response = await httpsClient.get(
        "/api/weedout/$businessId",
      );
      event = KnockOutEvent.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
