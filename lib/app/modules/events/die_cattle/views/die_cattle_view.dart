...
import 'package:intellectual_breed/app/widgets/cell_image.dart';
...
class DieCattleView extends GetView<DieCattleController> {
  const DieCattleView({Key? key}) : super(key: key);
  ...
  Widget _operationInfo(context) {
    return MyCard(children: [
      ...
      CellImage(
          ...
          hint: '拍照记录',
          selectCount: 3,
          showImgs: List.from(controller.imgsList.value),
          onChanged: (List<String> imgs) {
            print('--> $imgs');
            controller.imgsList.value = imgs;
          }),
      CellTextArea(
        isRequired: false,
        title: "备注信息",
