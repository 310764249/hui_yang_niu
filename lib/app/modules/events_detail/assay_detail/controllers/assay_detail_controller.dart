...
import 'package:intellectual_breed/app/models/cattle_event.dart';
...
class AssayDetailController extends GetxController {
  //传入的参数
  ...
  void onInit() {
    ...
    cdList = AppDictList.searchItems('xycd') ?? [];
    //处理传入参数
    handleArgument();
  }
  ...
  void handleArgument() async {
    ...
    if (argument is SimpleEvent) {
      ...
      print('handleArgument---${argument.cowId}');
      //生产记录传入的
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      var response = await httpsClient.get(
        "/api/breedmeasure/$businessId",
      );
      event = AssayArgument.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
