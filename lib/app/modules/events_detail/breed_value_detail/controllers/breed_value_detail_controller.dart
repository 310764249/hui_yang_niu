...
class BreedValueDetailController extends GetxController {
  //传入的参数
  ...
  void handleArgument() async {
    ...
    if (argument is SimpleEvent) {
      ...
      cattle = await CommonService().getCattleDetail(event!.cowId);
    } else if (argument is CattleEvent) {
      //生产记录传入的
      CattleEvent cattleEvent = argument;
      cattle = await CommonService().getCattleDetail(cattleEvent.cowId!);
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      var response = await httpsClient.get(
        "/api/breedvaluestats/$businessId",
      );
      event = BreedValueEvent.fromJson(response);
    } catch (error) {
      Toast.dismiss();
