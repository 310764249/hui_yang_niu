...
class ChangeGroupController extends GetxController {
  //TODO: Implement ChangeGroupController
  //传入的参数
  ...
  void handleArgument() async {
    ...
    if (argument is Cattle) {
      ...
      //   if (event!.inCowHouseId == element.id) {
      //     //更新栋舍容纳量
  ...
  void newAction() async {
    ...
    try {
      ...
      Map<String, dynamic> para = {
        ...
        'date': timesStr.value, // string 转群时间
        'remark': remarkController.text.trim(), // 备注
      };
      //print(para);
      await httpsClient.post("/api/turngroup", data: para);
      Toast.dismiss();
  ...
  void editAction() async {
    ...
    try {
      ...
      //print(para);
      await httpsClient.put("/api/turngroup", data: para);
      Toast.dismiss();
      Toast.success(msg: '提交成功');
      Get.back();
    } catch (error) {
