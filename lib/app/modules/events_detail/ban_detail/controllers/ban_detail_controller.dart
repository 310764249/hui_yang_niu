...
class BanDetailController extends GetxController {
  //TODO: Implement BanDetailController
  //传入的参数
  ...
  void onInit() {
    ...
    passList = AppDictList.searchItems('jpyy') ?? [];
    //处理传入参数
    handleArgument();
  }
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      ...
      event = BanEvent.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
