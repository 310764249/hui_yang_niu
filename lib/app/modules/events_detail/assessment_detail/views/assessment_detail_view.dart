...
class AssessmentDetailView extends GetView<AssessmentDetailController> {
  const AssessmentDetailView({Key? key}) : super(key: key);
  ...
  Widget _operationInfo(context) {
    return MyCard(children: [
      ...
      CellButtonDetail(
        isRequired: true,
        title: '前胸',
        hint: controller.findLabelByCode(controller.event!.forebreast),
      ),
      CellButtonDetail(
        isRequired: true,
        title: '四肢',
        hint: controller.findLabelByCode(controller.event!.allFours),
      ),
      CellButtonDetail(
        isRequired: true,
        title: '站立姿势',
        hint: controller.findLabelByCode(controller.event!.stance),
      ),
      CellButtonDetail(
        isRequired: true,
        title: '驱臀',
        hint: controller.findLabelByCode(controller.event!.hipDrive),
      ),
      controller.cattle!.gender == 1
          ? CellButtonDetail(
              isRequired: true,
              title: '睾丸',
              hint: controller.findLabelByCode(controller.event!.testis),
            )
          : const SizedBox(),
      controller.cattle!.gender == 2
          ? CellButtonDetail(
              isRequired: true,
              title: '乳房',
              hint: controller.findLabelByCode(controller.event!.breast),
            )
          : const SizedBox(),
      controller.cattle!.gender == 2
          ? CellButtonDetail(
              isRequired: true,
              title: '阴户',
              hint: controller.findLabelByCode(controller.event!.vulva),
            )
          : const SizedBox(),
      CellButtonDetail(
        isRequired: true,
        title: '遗传缺陷',
        hint: controller.findLabelByCode(controller.event!.geneticDefect),
      ),
      CellButtonDetail(
        isRequired: true,
        title: '综合评分',
        hint: (controller.event!.score!.toInt()).toString(),
      ),
      CellButtonDetail(
        isRequired: false,
        title: '操作人',
