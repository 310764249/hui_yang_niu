...
class SellCattleDetailController extends GetxController {
  //TODO: Implement SellCattleDetailController
    //传入的参数
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      var response = await httpsClient.get(
        "/api/market/$businessId",
      );
      event = SellArgument.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
