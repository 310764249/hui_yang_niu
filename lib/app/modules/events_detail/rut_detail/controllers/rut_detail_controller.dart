...
class RutDetailController extends GetxController {
  //TODO: Implement RutDetailController
  //传入的参数
  ...
  void onInit() {
    ...
    fqList = AppDictList.searchItems('fqbx') ?? [];
  ...
  Future<void> getCattleEvent(String businessId) async {
    try {
      ...
      event = RutArgument.fromJson(response);
    } catch (error) {
      Toast.dismiss();
      if (error is ApiException) {
        // 处理 API 请求异常情况 code不为 0 的场景
